...
- **2026-09-16** — **Auditoria: nada de escrito se perde por interrupção (pedido da Jo depois do caso do chipping).** Revisto o que cada ecrã guardava vs. o que só vivia em memória. A Jo escolheu corrigir **tudo** e **avisar** antes de sobrepor uma sessão por acabar.
  ...
  **3. Editor de campos** (`courses.tsx`): novo `DraftCourseEdit` por campo/tee (`draftCourseEdit:<courseId>:<tee|holes>`) — formulário de tee (par, CR, Slope, F9/B9) e a tabela dos 18 buracos gravados a cada tecla, repostos ao reabrir com banner; limpos ao gravar e ao **cancelar** (cancelar é deitar fora de propósito).
  **4. Range Drill:** o clube e a distância escritos mas ainda não adicionados vão no rascunho (`DraftRangeDrill.pendingShot`), e o autosave passou a correr também na fase **complete**, por isso as notas escritas aí já não se perdem.
  **5. Aviso de sessão por acabar** (`session.tsx`): ao abrir uma sessão sem `resume=1`, se houver rascunho com conteúdo pergunta — mesmo tipo: "Resume / Start new (discards)"; tipo diferente: "Open it / Discard it" (abre a outra com `resume=1`). O `applyDraft()` passou a ser partilhado pelos dois caminhos.
  tsc + eslint limpos. Não testado no browser — a Jo testa no deploy.
